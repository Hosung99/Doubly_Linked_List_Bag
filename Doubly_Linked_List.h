...
void list_head_insert(Node* head_ptr,int& entry) { //head부분에 새로 노드를 삽입 해주는 함수이다.
	Node* new_node=new Node(entry);    //새로운 노드를 만든다.
	new_node->next_ptr = head_ptr->next_ptr; //새로운 노드가 가리키는곳은 헤드포인터가 가리키는 곳이다.
	new_node->prev_ptr = NULL; //새로운 노드의 이전이 가리키는곳은 없다.
	head_ptr->next_ptr = new_node;		//헤드포인터는 새로운 만들어진 노드를  가리킨다.
	head_ptr->prev_ptr = NULL;          //헤드포인터의 이전이 가리키는곳은 없다.
